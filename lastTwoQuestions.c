...
//
//Obtiene los resultados ultimas dos preguntas respondidas
char* getTwoPreQuestionsLastPLayer(int game){
	sqlite3 *db = openDatabase();
  char sql[1024], out[2048];
	char *buffer;
  int rc;
	sprintf(sql, "select QuestionsPerGame.id_question from QuestionsPerGame where QuestionsPerGame.id_game = %d limit 2 offset (select count(*) from QuestionsPerGame where QuestionsPerGame.id_game = %d)-4;", game, game);
	rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	if (rc != SQLITE_OK)
  {
    fprintf(stderr, "Failed get questions: %s\n", sqlite3_errmsg(db));
